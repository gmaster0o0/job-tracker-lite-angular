...
- Applications → `apps/<name>`. Check workspace globs (for npm this is `workspaces` in root `package.json`) for an existing `apps/*` entry.
...
`nx import` can add the imported directory itself (e.g. `apps`) rather than package-level globs. Cross-package imports can then fail with `Cannot find module`.
...
- Source workspace config files — can conflict with destination settings
...
Install `typescript-eslint` explicitly so dependency resolution is deterministic across workspaces.
...
`@nx/eslint` may peer-depend on ESLint 8, causing the wrong version to resolve. If lint fails with `Cannot read properties of undefined (reading 'allow')`, add root `overrides`:
...
`nx import` may bring `node_modules/` and source lock/config files into the imported subtree. These are stale.
