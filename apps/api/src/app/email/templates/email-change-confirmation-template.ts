...
const templates: Record<
  ...
  {
    subject: string;
    text: (url: string, expiresInHours: number) => string;
    html: (url: string, expiresInHours: number) => string;
  }
> = {
  hu: {
    subject: 'Email-cím változtatás megerősítése - Job Tracker Lite',
    text: (url: string, expiresInHours: number) =>
      `Kérted, hogy megváltoztasd a Job Tracker Lite fiókod email címét. A változtatást ezen a linken erősítsd meg: ${url}. A link ${expiresInHours} órán belül lejár.`,
    html: (url: string, expiresInHours: number) => `
      <p>Szia!</p>
      ...
      <p>Ez a link ${expiresInHours} órán belül lejár.</p>
  ...
  en: {
    ...
    text: (url: string, expiresInHours: number) =>
      `You requested to change your Job Tracker Lite account email. Confirm the change using this link: ${url}. This link expires in ${expiresInHours} hours.`,
    html: (url: string, expiresInHours: number) => `
      <p>Hello!</p>
...
export function getEmailChangeConfirmationTemplate(
  ...
  return {
    ...
    text: template.text(url, expiresInHours),
    html: template.html(url, expiresInHours),
  };
}
